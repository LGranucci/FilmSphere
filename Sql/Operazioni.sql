...
set _serverTop = (
	select Codice
    from Server
    Where (Calcola_Distanza(Server.Latitudine, Server.Longitudine,UtenteLatitudine,UtenteLongitudine)  = 
			(select min(calcola_Distanza(Server.Latitudine, Server.Longitudine, UtenteLatitudine, UtenteLongitudine))
			from Server))
            LIMIT 1);
...
set _server2 = (
	select Codice
    from Server
    ORDER BY Calcola_Distanza(Server.Latitudine, Server.Longitudine,UtenteLatitudine,UtenteLongitudine)
    LIMIT 1 OFFSET 1);
set _server3 = (
	select Codice
    from Server
